...
export default function AccountWorkspacePage() {
  ...
  const coordinatedCadence = useMemo(() => {
    ...
    const staleActive = contacts.filter((c) => {
      ...
      const days = Math.floor(
        (Date.now() - new Date(c.last_activity_at).getTime()) /
          (1000 * 60 * 60 * 24)
      );
  ...
  async function runSequencingAction(
    ...
    if (kind === "work_due_now") {
      const dueNowContact = (
        data?.sequencing_intelligence?.recommended_contacts ?? []
      ).find((c) => c.why_next === "Cadence due now");
  ...
  if (err && !data) {
    return (
      <div className="space-y-4 p-6">
        ...
        <button className="crm-button px-4 py-2" onClick={() => router.push("/accounts")}>
          Back to Accounts
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <div className="crm-card overflow-hidden p-0">
        <div className="border-b border-white/70 bg-gradient-to-r from-white/95 via-white/85 to-violet-50/80 px-6 py-6">
      ...
      <div className="crm-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {busyAction === "work_hottest_contact"
                ? "Working..."
                : "Work Hottest Contact"}
            </button>
            ...
            >
              {busyAction === "create_account_task"
                ? "Creating..."
                : "Create Account Task"}
            </button>
      ...
      <div className="crm-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-lg font-semibold text-slate-900">
              Multi-Thread Sequencing Intelligence
            </div>
            <div className="mt-1 text-sm text-slate-600">
              Account mode:{" "}
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-4">
          ...
          <div className="crm-card-soft p-4">
            <div className="text-xs text-slate-500">Stale Active</div>
            <div className="mt-2 text-2xl font-semibold text-slate-900">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="crm-card-soft p-4">
            <div className="text-sm font-semibold text-slate-900">Rationale</div>

            <div className="mt-3 space-y-2">
              {data.sequencing_intelligence.rationale.length === 0 ? (
                <div className="text-sm text-slate-500">No rationale available.</div>
              ) : (
            ...
            <div className="mt-3 space-y-2">
              {data.sequencing_intelligence.risk_flags.length === 0 ? (
                <div className="text-sm text-slate-500">No major risk flags.</div>
              ) : (
                ...
                data.sequencing_intelligence.recommended_contacts.map((c) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="rounded-full border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700">
                        {c.priority_score}
                      </div>
                    </div>
      ...
      <div className="crm-card p-5">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-4">
          <div className="crm-card-soft p-4">
            <div className="text-xs text-slate-500">Due Now</div>
            <div className="mt-2 text-2xl font-semibold text-slate-900">
              {coordinatedCadence.dueNow.length}
            </div>
          </div>
          ...
          <div className="crm-card-soft p-4">
            <div className="text-xs text-slate-500">Uncovered</div>
            <div className="mt-2 text-2xl font-semibold text-slate-900">
              {coordinatedCadence.uncovered.length}
            </div>
          </div>
          ...
          <div className="crm-card-soft p-4">
            <div className="text-xs text-slate-500">Stale Active</div>
            <div className="mt-2 text-2xl font-semibold text-slate-900">
              {coordinatedCadence.staleActive.length}
            </div>
          </div>
          ...
          <div className="crm-card-soft p-4">
            ...
            <div className="mt-2 text-2xl font-semibold text-slate-900">
              {coordinatedCadence.blocked.length}
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-3">
          <div className="crm-card-soft p-4">
            <div className="text-sm font-semibold text-slate-900">Due Now</div>

            <div className="mt-3 space-y-3">
              {coordinatedCadence.dueNow.length === 0 ? (
                <div className="text-sm text-slate-500">No stakeholders due now.</div>
              ) : (
          ...
          <div className="crm-card-soft p-4">
            <div className="text-sm font-semibold text-slate-900">Uncovered</div>

            <div className="mt-3 space-y-3">
              {coordinatedCadence.uncovered.length === 0 ? (
                <div className="text-sm text-slate-500">No uncovered stakeholders.</div>
              ) : (
          ...
          <div className="crm-card-soft p-4">
            <div className="text-sm font-semibold text-slate-900">Stale Active</div>

            <div className="mt-3 space-y-3">
              {coordinatedCadence.staleActive.length === 0 ? (
                <div className="text-sm text-slate-500">No stale active sequences.</div>
              ) : (
      ...
      <div className="crm-card p-5">
        <div className="text-lg font-semibold text-slate-900">Gap Analysis</div>

        {data.gaps.length === 0 ? (
          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            No major gaps detected. Buying committee coverage and account motion
            look healthy.
          </div>
        ) : (
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="crm-card p-5">
          ...
          <div className="mt-4 space-y-3">
            {data.timeline.length === 0 ? (
              <div className="text-sm text-slate-500">No account activity found.</div>
            ) : (
              data.timeline.slice(0, 20).map((item) => (
                <div key={item.id} className="crm-card-soft p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-slate-900">
                    ...
                    <div className="rounded-full border border-slate-200 bg-white px-2 py-1 text-xs capitalize text-slate-700">
                      {item.kind}
                    </div>
                  </div>
        ...
        <div className="crm-card p-5">
          ...
          <div className="mt-4 space-y-3">
            {openTasks.length === 0 ? (
              <div className="text-sm text-slate-500">No open account tasks.</div>
            ) : (
              openTasks.slice(0, 20).map((task) => (
                <div key={task.id} className="crm-card-soft p-4">
                  <div className="text-sm font-semibold text-slate-900">
                    {task.title || "(No title)"}
                  </div>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <button
                      className="crm-button px-3 py-2 text-xs"
                      onClick={() =>
                        router.push(`/work?task_id=${encodeURIComponent(task.id)}`)
                      }
                    >
                      Open in Work Queue
                    </button>
