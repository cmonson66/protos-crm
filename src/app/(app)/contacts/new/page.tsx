...
export default function NewContactPage() {
  ...
  const [vertical, setVertical] = useState<ContactVertical>("coaching");
  ...
  useEffect(() => {
    async function loadCadences() {
      ...
      const res = await fetchWithAuth(`/api/cadence/list?vertical=${encodeURIComponent(vertical)}`);
      const json = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await fetchWithAuth(`/api/schools/search?q=${encodeURIComponent(q)}`);
      const json = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await fetchWithAuth(`/api/accounts/search?q=${encodeURIComponent(q)}`);
      const json = await res.json().catch(() => ({}));
  ...
  const showSchoolResults = useMemo(() => {
    return vertical === "coaching" && schoolQuery.trim().length > 0 && !selectedSchool;
  }, [vertical, schoolQuery, selectedSchool]);

  const showAccountResults = useMemo(() => {
    return vertical === "corporate" && accountQuery.trim().length > 0 && !selectedAccount;
  ...
  async function createContact() {
    ...
    const res = await fetchWithAuth("/api/contacts/create", {
      ...
      body: JSON.stringify({
        vertical,
        school_id: vertical === "coaching" ? selectedSchool?.id ?? null : null,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-3xl font-semibold">New Contact</div>
          <div className="mt-1 text-sm text-muted-foreground">
            Add a new prospect or contact into the CRM.
          </div>
        </div>
          ...
          {duplicateContact?.id ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <div className="mt-2 text-sm text-amber-800">
                ...
                {duplicateContact.school_name ? ` • ${duplicateContact.school_name}` : ""}
      ...
      <div className="rounded-2xl border p-5">
        <div className="text-lg font-semibold">Contact Type</div>

        <div className="mt-4 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={() => setVertical("coaching")}
            className={`rounded-xl border px-4 py-2 ${
              vertical === "coaching" ? "bg-slate-100 font-semibold" : ""
            }`}
          >
            Coaching
          </button>
          ...
          <button
            ...
            className={`rounded-xl border px-4 py-2 ${
              vertical === "corporate" ? "bg-slate-100 font-semibold" : ""
            }`}
          >
            Corporate
          </button>
        ...
        <div className="mt-3 text-sm text-muted-foreground">
          {vertical === "coaching"
            ? "Coaching contacts attach to a school and only show coaching cadences."
            : "Corporate contacts attach to an account/company and only show corporate cadences."}
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-6">
          <div className="rounded-2xl border p-5 space-y-4">
            ...
            <input
              className="w-full rounded-xl border px-3 py-2"
              placeholder={vertical === "corporate" ? "Job title / role" : "Job title"}
              value={jobTitle}
                        ...
                        {schoolLoading ? (
                          ...
                          <>
                            ...
                            {!exactSchoolMatch && schoolQuery.trim() ? (
                              ...
                              >
                                Create school: <span className="font-semibold">{schoolQuery.trim()}</span>
                              </button>
                            ) : null}
                          </>
                        )}
                          ...
                          <>
                            ...
                            {!exactAccountMatch && accountQuery.trim() ? (
                              <button
                                ...
                                className="block w-full px-3 py-2 text-left text-sm font-medium hover:bg-slate-50 disabled:opacity-50"
                              >
                                Create account: <span className="font-semibold">{accountQuery.trim()}</span>
                              </button>
                            ) : null}
                          </>
                        )}
          ...
          {vertical === "corporate" ? (
            <div className="rounded-2xl border p-5 space-y-4">
              <div className="text-lg font-semibold">Corporate Buying Context</div>
              ...
              <label className="flex items-center gap-3 rounded-xl border px-3 py-2">
                ...
                />
                <span className="text-sm">Budget owner</span>
              </label>
            </div>
          ) : null}
        </div>
        ...
        <div className="rounded-2xl border p-5 space-y-4">
          <div className="text-lg font-semibold">
            {vertical === "coaching" ? "Program / Pipeline" : "Pipeline"}
          </div>

          <input
            className="w-full rounded-xl border px-3 py-2"
            placeholder={vertical === "coaching" ? "Sport" : "Market / Segment / Focus"}
            value={sport}
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <input
              className="rounded-xl border px-3 py-2"
              placeholder={vertical === "coaching" ? "Division" : "Division / Business Unit"}
              value={division}
              onChange={(e) => setDivision(e.target.value)}
            />
            <input
              className="rounded-xl border px-3 py-2"
              placeholder={vertical === "coaching" ? "Conference" : "Industry / Subsector"}
              value={conference}
              onChange={(e) => setConference(e.target.value)}
            />
            <input
              className="rounded-xl border px-3 py-2"
              placeholder={vertical === "coaching" ? "Region" : "Region / Territory"}
              value={region}
